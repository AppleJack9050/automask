...
async def get_files():
    ...
    saved_files = os.listdir(edited_file_directory)
...
async def show_editor(file_name: str):
    ...
    base_image = file_handler.return_base_image(file_path, file_name)
...
async def download_file(file_name: str):
    try:
        file_path = os.path.join(processed_file_directory, file_name)
        if not os.path.exists(file_path):
            return JSONResponse({"error": "File not found"}, status_code=404)

        file_handler.download(file_path)
